...
test('все экраны наследуют единую оболочку', () => {
  ...
  expect(routeObjects[0].children?.map(({ path, index }) => index ? '(index)' : path)).toEqual([
    '(index)', 'play/:sessionId', 'studio', 'studio/stories/:storyId/characters', 'studio/:sessionId', 'characters', 'characters/:characterId', 'characters/:storyId/:characterId', 'settings',
  ])
})
